...
ws.onmessage = (event) => {
    ...
    if (data.type === 'balance') {
        // Format XRP balance with 3 decimal places
        const xrpBalance = parseFloat(data.xrpBalance).toFixed(3);
        xrpBalanceElement.textContent = `${xrpBalance} XRP`;
        console.log('XRP Balance set to:', xrpBalanceElement.textContent); // Debug log
...
if (sendPayOption) {
    sendPayOption.addEventListener('click', () => {
        ...
        if (sendPayContent) sendPayContent.classList.add('active');
        if (document.getElementById('requestContent')) document.getElementById('requestContent').classList.remove('active');
...
if (requestOption) {
    requestOption.addEventListener('click', () => {
        ...
        if (document.getElementById('requestContent')) document.getElementById('requestContent').classList.add('active');
...
if (sendButton) {
    sendButton.addEventListener('click', async () => {
        ...
        const recipient = recipientAddressInput ? recipientAddressInput.value : '';
        const amount = sendAmountInput ? sendAmountInput.value : '';
        const currency = sendCurrencySelect ? sendCurrencySelect.value : '';
